    ...
    addLines([
      {
        ...
        variantOptions: product.variants.filter((variant) => variant.available > 0).map((variant) => ({
          ...
          color: variant.color,
          unitPrice: product.price,
          availability: variant.available <= 2 ? "low_stock" as const : "in_stock" as const,
        })),
      },
    ]);
    setNotice(isRu ? "Demo-позиция добавлена." : "Demo позиция қосылды.");
    ...
    <section className="demo-pdp" data-testid={`demo-product-${product.slug}`}>
      ...
      <div className="demo-pdp__layout">
        <div className="demo-pdp__media">
          ...
          />
          <span>DEMO / {product.category}</span>
        </div>
        <div className="demo-pdp__details">
          <div className="demo-pdp__head">
            <p className="q-meta">{product.category} / GRAPHITE DEMO</p>
            <h1>{product.name}</h1>
            <p>{product.description}</p>
            <strong>{formatKzt(product.price, locale)}</strong>
          </div>
          <fieldset className="demo-size-picker">
            <legend>{isRu ? "Размер" : "Өлшем"}</legend>
            <div>
          ...
          <button
            className="q-button q-button--solid demo-pdp__add"
            disabled={!selected}
            onClick={addToCart}
            type="button"
          >
            {isRu ? "Добавить demo-позицию" : "Demo позицияны қосу"}
            <span aria-hidden="true">→</span>
          </button>
          <p aria-live="polite" className="q-status" data-kind="success">{notice}</p>
          <Link className="q-text-link" href={`/${locale}/build-a-set?demo=1`}>
            {isRu ? "Подобрать второй модуль" : "Екінші модульді таңдау"}
          </Link>
          <dl className="demo-pdp__facts">
            <div><dt>DATA SOURCE</dt><dd>SEEDED DEMO FIXTURE</dd></div>
            <div><dt>PAYMENT</dt><dd>DEVELOPMENT MOCK ONLY</dd></div>
            <div><dt>PUBLIC VISIBILITY</dt><dd>DISABLED</dd></div>
          </dl>
        </div>
      </div>
    </section>
