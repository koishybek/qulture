...
import { cleanupE2EOrders, dismissConsentIfPresent, preparePublicSession } from "./helpers";

test.afterAll(cleanupE2EOrders);

test("isolated demo supports independent sizes through signed order status", {
  tag: "@desktop",
}, async ({ page }) => {
  await preparePublicSession(page, true);
  await page.goto("/ru/build-a-set?demo=1");
  ...
  const drawer = page.getByRole("dialog", { name: /Корзина \(2\)/ });
  await expect(drawer).toBeVisible();
  await expect(drawer).toContainText("TOP");
  await expect(drawer).toContainText("PANTS");
  const topLineSize = drawer.locator(".cart-line").filter({ hasText: "TOP" }).locator("select");
  ...
  await drawer
    .getByRole("link", { name: /Открыть корзину/ })
    .click();
  ...
  await page
    ...
    .fill("+77001234567");
  await page.getByLabel("Адрес", { exact: true }).fill("Astana, Demo street 1");
  await page
    .getByRole("checkbox", { name: /Принимаю draft Terms of Sale/ })
    .check();
  await page
    .getByRole("checkbox", { name: /Согласен на сервисные уведомления/ })
    .check();
  ...
  await expect(page).toHaveURL(
    /\/ru\/order-status\?order=Q-D[^#]+#token=qot_/,
  );
  await expect(
